...
def plot_line(X, y, weights, epoch, time):
    ...
    plt.plot(xx, yy)
    plt.title('W = ({0:.2f},{1:.2f}) | b = {2:.2f}'.format(
...
def main(data):
    ...
    if data == None:
        ...
        X, y = make_classification(
            n_samples=30, n_features=2, n_redundant=0, n_clusters_per_class=1, class_sep=0.9)
    else:
        print('Loading data...')
    ...
    print('W = ({0:.2f},{1:.2f}) | b = {2:.2f}'.format(
        weights[1], weights[2], weights[0]))
    ...
    while correct < len(X) and epoch < 100:
        ...
        if gt == prediction:
            ...
            print('\nUpdate Weights:')
            print('  Weights = ({0:.2f},{1:.2f}) | bias = {2:.2f}'.format(
                weights[1], weights[2], weights[0]))
