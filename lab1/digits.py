...
if __name__ == "__main__":
    ...
    avg_x, avg_y, med_x, med_y, max_x, max_y = get_sizes('digits/data')
    base_x = 10
    base_y = 20
    images = load_images('digits/data', fout, int(base_x), int(base_y))
    # images = load_images('digits/data', fout, int(med_x), int(med_y))
    # images = load_images('digits/data', fout, int(max_x), int(max_y))
    fout.close
    end_time = time.time()
